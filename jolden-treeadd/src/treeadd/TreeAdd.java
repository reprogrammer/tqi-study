 ...
 * <p>
 * Treeadd is a very simple program that performs a recursive depth first
 * traversal of a binary tree and sums the value of each element in the tree. We
 * initialize the elements in the tree to contain '1'.
 **/
public class TreeAdd {
	...
	public static void main(String args[]) {
		...
		if (printMsgs) {
			System.out
					.println("Treeadd alloc time " + (end0 - start0) / 1000.0);
			System.out.println("Treeadd add time " + (end1 - start1) / 1000.0);
			System.out
					.println("Treeadd total time " + (end1 - start0) / 1000.0);
		}
			...
			if (arg.equals("-l")) {
				if (i < args.length) {
					...
					throw new RuntimeException(
							"-l requires the number of levels");
